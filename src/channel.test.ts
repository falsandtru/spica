...
describe('Unit: lib/channel', function () {
  describe('channel', function () {
    it('buffer 0', async function () {
      ...
      assert(await Promise.race([new Channel().send(), wait(1).then(() => -1)]) === -1);
      await (async ch => {
        assert(await Promise.race([ch.send(0), wait(1).then(() => -1)]) === -1);
        assert(await ch.take() === 0);
        assert(await Promise.race([ch.take(), wait(1).then(() => -1)]) === -1);
      })(new Channel<number>());
      await (async ch => {
        assert(await Promise.race([ch.take(), wait(1).then(() => -1)]) === -1);
        assert(await ch.send(0) === undefined);
        assert(await Promise.race([ch.send(0), wait(1).then(() => -1)]) === -1);
      })(new Channel<number>());
    });
    ...
    it('buffer 1', async function () {
      ...
      assert(await Promise.race([new Channel(1).send(), wait(1).then(() => -1)]) === undefined);
      await (async ch => {
        assert(await Promise.race([ch.send(0), wait(1).then(() => -1)]) === undefined);
        assert(await ch.take() === 0);
        assert(await Promise.race([ch.take(), wait(1).then(() => -1)]) === -1);
