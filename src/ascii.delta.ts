...
// Contextual opportunistic delta encoding
...
肥大化リスクがなく安全かつ高圧縮率であるため普遍的に使用可能。
...
エントロピー符号の最適性の根拠となるシャノンエントロピーは単一のIID空間に対して最適な
符号化を追及するものであることからその最適性は分布に傾向がないゆえに区分する意味のない
単一空間としておよびデータが互いに独立であるゆえに統計的にという非現実的な前提に基づく
何重もの不要な限定条件下での最適性であり最適な符号化を既知のデータに対して算出するのでなく
未知のデータに対して予測する場合に予測との誤差に対してまで最適となるものでもない。
エントロピー符号は符号化空間を2x2、中心の1x1の空間が効率的であるとすると
残部が非効率または非常に非効率となり誤差に対して大きなペナルティを負うが1x1x4のように
空間そのものを分割縮小することで圧縮率の向上とペナルティの軽減が可能である。
...
2文字以上の文字列での出現率に基づいて符号化すべきだがデータがないので
1文字での出現率の組み合わせに簡易化(圧縮率はあと5%前後上がるだろう)。
出力も可変長化するなら算術符号化すべきだろう。
...
ANS/FSE:
単純な直接適用ではあまり圧縮率が上がらないが調整すれば最高圧縮率になる可能性がある。
算術符号は入力の圧縮単位が可変長であることおよび圧縮可能なパターンに制約がないことから
固定長かつ制約付きのDASCIIより理論的には明らかに効率的である。
しかし出力がバイト境界を溢れることで短い入力において非効率となる可能性が高い。
英文中の出現率におけるアルファベット26文字の理論上の最大圧縮率は
最小エントロピー約4.1bitから約48.75%と計算できるがバイト境界を考慮した圧縮率は
2文字以下が0%、3文字が33%、4文字が25%に著しく低下し全体の圧縮率を低下させるため
バイト境界への対応なしに優れた圧縮率は得られないだろう。
さらにオーバーヘッドがハフマン符号より大きいことも短い入力に対する懸念材料となる。
