...
ハフマン符号、算術符号、範囲符号などのエントロピー符号は情報源の分布(確率、規則性)のみを
使用して最小エントロピー(シャノンエントロピー、最大圧縮率)を追及するものであるため理論上
の最小エントロピーに到達したとしてもさらに圧縮率の高い圧縮方法がないことを意味しない。
最小エントロピーは単一の規則により突破できる場合と複数の規則により突破できる場合があるが
特に後者は容易であるにもかかわらず基礎理論の学習において併記され知る機会がほとんどなく
その分野で他に適している手法があることが知られている場合を除き最小エントロピーに到達
または近似すればゴールであり十分であると誤信されておりさらに先に進む展望を奪っている。
この誤信は最小エントロピーの誤謬、シャノンエントロピーの誤謬などと言うことができよう。
実践的な圧縮エンコーディングは最小エントロピーを突破するのがスタートラインである。
現在のCS教育および関連資料は実践的圧縮エンコーディングの作成のみならず評価に必要な基礎
をも欠いていると言える。
DAは複数の規則の平均すなわち全体としては最小エントロピーを突破しており英単語および英文
に対する単一の規則においてもおそらく突破しているまたは突破可能である。
