  ...
  describe('Cache', function () {
    ...
    for (const length of [1e1, 1e2, 1e3, 1e4, 1e5, 1e6]) {
      it(`Clock set ${length.toLocaleString('en')} 0%`, function (done) {
        const cache = new Clock<number, object>(length);
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`Clock    set ${length.toLocaleString('en')} 0%`, () => cache.set(random() * -length - 1 | 0, {}), done);
      });

      it(`ISC set ${length.toLocaleString('en')} 0%`, function (done) {
        const cache = new LRUCache<number, object>({ max: length });
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`ISCCache set ${length.toLocaleString('en')} 0%`, () => cache.set(random() * -length - 1 | 0, {}), done);
      });

      it(`LRU set ${length.toLocaleString('en')} 0%`, function (done) {
        const cache = new LRU<number, object>(length);
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`LRUCache set ${length.toLocaleString('en')} 0%`, () => cache.set(random() * -length - 1 | 0, {}), done);
      });

      it(`DWC set ${length.toLocaleString('en')} 0%`, function (done) {
        const cache = new Cache<number, object>(length);
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`DW-Cache set ${length.toLocaleString('en')} 0%`, () => cache.set(random() * -length - 1 | 0, {}), done);
      });
    }
    ...
    for (const length of [1e1, 1e2, 1e3, 1e4, 1e5, 1e6]) {
      it(`Clock set ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`Clock    set ${length.toLocaleString('en')} 100%`, () => cache.set(random() * length | 0, {}), done);
      });
      ...
      it(`ISC set ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`ISCCache set ${length.toLocaleString('en')} 100%`, () => cache.set(random() * length | 0, {}), done);
      });
      ...
      it(`LRU set ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`LRUCache set ${length.toLocaleString('en')} 100%`, () => cache.set(random() * length | 0, {}), done);
      });
      ...
      it(`DWC set ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`DW-Cache set ${length.toLocaleString('en')} 100%`, () => cache.set(random() * length | 0, {}), done);
      });
    }
    ...
    for (const length of [1e1, 1e2, 1e3, 1e4, 1e5, 1e6]) {
      it(`Clock get ${length.toLocaleString('en')} 0%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`Clock    get ${length.toLocaleString('en')} 0%`, () => cache.get(random() * -length - 1 | 0), done);
      });
      ...
      it(`ISC get ${length.toLocaleString('en')} 0%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`ISCCache get ${length.toLocaleString('en')} 0%`, () => cache.get(random() * -length - 1 | 0), done);
      });
      ...
      it(`LRU get ${length.toLocaleString('en')} 0%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`LRUCache get ${length.toLocaleString('en')} 0%`, () => cache.get(random() * -length - 1 | 0), done);
      });
      ...
      it(`DWC get ${length.toLocaleString('en')} 0%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`DW-Cache get ${length.toLocaleString('en')} 0%`, () => cache.get(random() * -length - 1 | 0), done);
      });
    }
    ...
    for (const length of [1e1, 1e2, 1e3, 1e4, 1e5, 1e6]) {
      it(`Clock get ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`Clock    get ${length.toLocaleString('en')} 100%`, () => cache.get(random() * length | 0), done);
      });
      ...
      it(`ISC get ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`ISCCache get ${length.toLocaleString('en')} 100%`, () => cache.get(random() * length | 0), done);
      });
      ...
      it(`LRU get ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`LRUCache get ${length.toLocaleString('en')} 100%`, () => cache.get(random() * length | 0), done);
      });
      ...
      it(`DWC get ${length.toLocaleString('en')} 100%`, function (done) {
        ...
        for (let i = 0; i < length; ++i) cache.set(i, {});
        const random = xorshift.random(1);
        benchmark(`DW-Cache get ${length.toLocaleString('en')} 100%`, () => cache.get(random() * length | 0), done);
      });
    }
    ...
    for (const length of [1e1, 1e2, 1e3, 1e4, 1e5, 1e6]) {
      it(`ISC simulation ${length.toLocaleString('en')} expire`, captureTimers(function (done) {
        const cache = new LRUCache<number, object>({ max: length, ttl: 1, ttlAutopurge: true });
        const age = (r => () => r() * 1e3 | 0)(xorshift.random(1));
        for (let i = 0; i < length; ++i) cache.set(i, {}, { ttl: age() });
        const random = xorshift.random(1);
      ...
      it(`DWC simulation ${length.toLocaleString('en')} expire`, function (done) {
        const cache = new Cache<number, object>(length, { age: 1, eagerExpiration: true });
        const age = (r => () => r() * 1e3 | 0)(xorshift.random(1));
        for (let i = 0; i < length; ++i) cache.set(i, {}, { age: age() });
        const random = xorshift.random(1);
